...
class SubscriptionService {
  ...
  async getUsageStats(subscriptionId) {
    ...
    return this.connectionManager.executeWithCache(
      cacheKey,
      async () => {
        const response = await this.connectionManager.prepare(SUBSCRIPTION_QUERIES.USAGE_STATS).bind(subscriptionId).first();
        return response || {
          id: subscriptionId,
          active_customers: 0,
          forms_created: 0,
      ...
      10 * 60
      // 10 minutes cache for usage stats
    );
  }
  async getAnalytics() {
  ...
  async getPopularFeatures() {
    ...
    return this.connectionManager.executeWithCache(
      ...
      30 * 60
      // 30 minutes cache for popular features
    );
  }
  async getSubscriptionLimits(subscriptionId) {
    ...
    return this.connectionManager.executeWithCache(
      cacheKey,
      async () => {
        const response = await this.connectionManager.prepare(SUBSCRIPTION_QUERIES.SUBSCRIPTION_LIMITS).bind(subscriptionId).first();
        if (response) {
  ...
  async create(subscriptionData) {
    ...
    ).bind(name, description, price, billing_cycle, trial_days, max_forms, max_submissions).run();
  ...
  async bulkCreateFeatures(subscriptionId, features) {
    if (!features.length) return;
    const operations = [];
    const featureIds = /* @__PURE__ */ new Map();
    for (const feature of features) {
      operations.push(
        this.connectionManager.prepare(SUBSCRIPTION_QUERIES.INSERT_FEATURE).bind(feature.name, feature.description || null)
      );
    }
    await this.connectionManager.batch(operations);
    const featureIdOperations = features.map(
      (feature) => this.connectionManager.prepare(SUBSCRIPTION_QUERIES.SELECT_FEATURE_ID).bind(feature.name)
    );
    ...
    for (const [featureName, featureId] of featureIds) {
      relationshipOperations.push(
        this.connectionManager.prepare(SUBSCRIPTION_QUERIES.INSERT_SUBSCRIPTION_FEATURE).bind(subscriptionId, featureId)
      );
    }
  ...
  async update(id, subscriptionData) {
    ...
    const response = await this.connectionManager.prepare(SUBSCRIPTION_QUERIES.UPDATE_SUBSCRIPTION).bind(name, description, price, billing_cycle, trial_days, max_forms, max_submissions, id).run();
    if (!response.success) {
      throw new Error("Failed to update subscription");
    }
    if (features) {
      await this.connectionManager.prepare(SUBSCRIPTION_QUERIES.DELETE_SUBSCRIPTION_FEATURES).bind(id).run();
  ...
  async clearSubscriptionCaches(subscriptionId) {
    const cacheKeys = [
      "subscriptions:all",
  ...
  async getHealthMetrics() {
    ...
    return this.connectionManager.executeWithCache(
      ...
      15 * 60
      // 15 minutes cache for health metrics
    );
  }
}

export { SubscriptionService as S };
//# sourceMappingURL=subscription_Dua9-wUJ.mjs.map
