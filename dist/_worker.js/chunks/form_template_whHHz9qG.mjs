...
import { D as D1ConnectionManager } from './d1-connection-manager_oVL7uFVJ.mjs';

const FORM_TEMPLATE_QUERIES = {
  BASE_SELECT: `
    SELECT
      ft.*,
...
const processTemplateResults = (connectionManager) => {
  return (rows) => {
    return rows.map((row) => {
      const template = { ...row };
...
class FormTemplateService {
  ...
  async getAll(filters) {
    ...
    return this.connectionManager.executeWithCache(
      ...
      async () => {
        ...
        let whereConditions = ["ft.is_active = true"];
        let bindParams = [];
        if (filters?.category) {
  ...
  async create(templateData) {
    ...
    const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.INSERT_TEMPLATE).bind(
      name,
      description || null,
      category,
      subcategory || null,
  ...
  async update(id, templateData) {
    ...
    const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.UPDATE_TEMPLATE).bind(
      name,
      description,
      category,
      subcategory,
  ...
  async delete(id) {
    const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.DELETE_TEMPLATE).bind(id).run();
    if (!response.success) {
  ...
  async publish(id) {
    const response = await this.connectionManager.prepare(`UPDATE form_templates SET is_published = true WHERE id = ?`).bind(id).run();
    if (!response.success) {
  ...
  async unpublish(id) {
    const response = await this.connectionManager.prepare(`UPDATE form_templates SET is_published = false WHERE id = ?`).bind(id).run();
    if (!response.success) {
  ...
  async getPopularTemplates(limit = 10) {
    ...
    return this.connectionManager.executeWithCache(
      cacheKey,
      async () => {
        const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.POPULAR_TEMPLATES).bind(limit).all();
        if (response.success) {
          return processTemplateResults(this.connectionManager)(response.results);
        }
        return [];
      ...
      30 * 60
      // 30 minutes cache for popular templates
    );
  }
  async getCategoryStatistics() {
    ...
    return this.connectionManager.executeWithCache(
      ...
      30 * 60
      // 30 minutes cache for category stats
    );
  }
  async bulkCreate(templates) {
    const statements = templates.map(
      (template) => this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.INSERT_TEMPLATE).bind(
        template.name,
  ...
  async createCollection(collectionData) {
    ...
    const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.CREATE_COLLECTION).bind(name, description || null, created_by, JSON.stringify(metadata || {})).run();
    if (!response.success) {
  ...
  async updateCollection(id, collectionData) {
    ...
    const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.UPDATE_COLLECTION).bind(name, description, JSON.stringify(metadata), id).run();
    if (!response.success) {
  ...
  async deleteCollection(id) {
    const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.DELETE_COLLECTION).bind(id).run();
    if (!response.success) {
  ...
  async getCollectionById(id) {
    ...
    return this.connectionManager.executeWithCache(
      cacheKey,
      async () => {
        const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.GET_COLLECTION_BY_ID).bind(id).first();
        if (response) {
          return {
            ...response,
  ...
  async addTemplateToCollection(collectionId, templateId, addedBy) {
    const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.ADD_TEMPLATE_TO_COLLECTION).bind(collectionId, templateId, addedBy).run();
    if (!response.success) {
  ...
  async removeTemplateFromCollection(collectionId, templateId) {
    const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.REMOVE_TEMPLATE_FROM_COLLECTION).bind(collectionId, templateId).run();
    if (!response.success) {
  ...
  async getCollectionTemplates(collectionId) {
    ...
    return this.connectionManager.executeWithCache(
      cacheKey,
      async () => {
        const response = await this.connectionManager.prepare(FORM_TEMPLATE_QUERIES.GET_COLLECTION_TEMPLATES).bind(collectionId).all();
        if (response.success) {
          return processTemplateResults(this.connectionManager)(response.results);
        }
        return [];
  ...
  async getFacets() {
    ...
    return this.connectionManager.executeWithCache(
      ...
      15 * 60
      // 15 minutes cache for facets
    );
  }
  async getTemplateWithTotalCount(filters) {
  ...
  async updateUsageStats(templateId, statsUpdate) {
...
export { FormTemplateService as F };
//# sourceMappingURL=form_template_whHHz9qG.mjs.map
